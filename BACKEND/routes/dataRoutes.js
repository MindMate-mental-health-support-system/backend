...
router.post('/process', upload.single('voice'), async (req, res) => {
  ...
  try {
    const { text, type, userId, language } = req.body;
    ...
    if (type === 'text') {
      ...
      if (text && text.trim() && !text.includes('transcription unavailable')) {
        ...
        userText = await TranscriptionService.transcribe(voiceFile, language);
      }
      console.log(`\n\n🎤 VOICE MESSAGE TRANSCRIBED:\n"${userText}"\n\n`);

      // Step 1b: Run SED on the raw audio file for emotion
      const voiceEmotionData = await EmotionService.detectVoiceEmotion(voiceFile);
      console.log('✓ Voice emotion detected (SED):', voiceEmotionData);
      
      const textEmotionData = await EmotionService.detectTextEmotion(userText);
      console.log('✓ Text emotion detected:', textEmotionData);

      emotionData = {
          emotion: voiceEmotionData.emotion,
          confidence: voiceEmotionData.confidence,
          textEmotion: textEmotionData.emotion,
          textEmotionConfidence: textEmotionData.confidence
      };
    }
    ...
    if (crisisAnalysis.isCrisis) {
      ...
      if (aiAvailable) {
        aiResponse = await AIContentService.generateContent({
          ...
          textEmotion: emotionData.textEmotion,
          textEmotionConfidence: emotionData.textEmotionConfidence,
      ...
      setTimeout(() => {
        try {
          fs.unlink(uploadedFile.path, (err) => {
            if (err && err.code !== 'ENOENT') console.error('Error cleaning up file:', err);
            else if (!err) console.log('✓ Uploaded file cleaned up:', uploadedFile.path);
          });
        } catch (err) {

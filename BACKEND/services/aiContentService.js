...
5. Keep it to 3-5 heart-felt, stabilizing sentences. Avoid clinical jargon but maintain a professional safety-first approach.`;
  ...
  static async callAIWithFallback(prompt, onChunk = null) {
    const providers = [
      ...
      { name: 'OpenRouter', call: () => AIContentService.callOpenAILike(prompt, process.env.OPENROUTER_API_KEY, 'https://openrouter.ai/api/v1/chat/completions', 'meta-llama/llama-3-8b-instruct:free', onChunk) },
